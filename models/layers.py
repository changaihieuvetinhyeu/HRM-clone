...
class Attention(nn.Module):
    ...
    def forward(self, cos_sin: CosSin, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        # Naive scaled dot-product attention (replace FlashAttention)
        scores = torch.einsum('bthd,bshd->bhts', query, key) / self.head_dim ** 0.5
        if self.causal:
            mask = torch.triu(torch.ones(seq_len, seq_len, device=scores.device), diagonal=1).bool()
            scores = scores.masked_fill(mask[None, None, :, :], float('-inf'))
        attn = F.softmax(scores, dim=-1)
        attn_output = torch.einsum('bhts,bshd->bthd', attn, value)
